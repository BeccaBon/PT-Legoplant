function scr_player_mach2()
{
	...
	if (input_buffer_jump < 8 && grounded && sprite_index != spr_secondjump1 && sprite_index != spr_clownjump && (!((move == 1 && xscale == -1))) && (!((move == -1 && xscale == 1))))
	{
        ...
        if (key_jump2 && key_up && (!ispeppino) && character == "P" && noisedoublejump)
	...
	if (key_down && !skateboarding && sprite_index != spr_playerN_grindcancel && !place_meeting(x, y, obj_dashpad))
	{
		particle_set_scale(particle.jumpdust, xscale, 1);
		create_particle(x, y, particle.jumpdust, 0);
		flash = false;
	...
	{
		if !ispeppino
		scr_soundeffect(sfx_machslideboostN)
		else
	...
	if (sprite_index == spr_rollgetup or sprite_index == spr_suplexdash or sprite_index == spr_longjump or sprite_index == spr_longjumpend)
		image_speed = 0.4
	else
